...
// updated_at: "2018-11-17 00:27:13"
  ...
  loadStore() {
    this.route.params.subscribe(params => {
      ...
      this.id = params.id;
      this.storeService.find(this.id)
        .subscribe(({data}: any) => {
  ...
  preloadStore(recipes: Array<any>, supplies: Array<any>, suppliers: Array<any>) {
  ...
  onSubmit() {
    ...
    data.supplies = this.data.supplies.filter(s => s.selected).map(s => ({
      supply_id: s.id,
      stock: s.stock,
    }));

    data.recipes = this.data.recipes.filter(s => s.selected).map(s => ({
      recipe_id: s.id,
    }));

    data.suppliers = this.data.suppliers.filter(s => s.selected).map(s => ({
      supplier_id: s.id,
    }));
  ...
  update(data) {
    this.storeService.update({...data, id: this.id})
      .subscribe((result) => {
        this.cancel()
      }, (error) => {
        alert(parseErrroMessage(error));
